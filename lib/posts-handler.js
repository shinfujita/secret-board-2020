    ...
    case 'GET':
      ...
      Post.findAll(
        ...
        posts.forEach(post => {
          ...
          post.formattedUpdatedAt = moment(post.updatedAt).tz('Asia/Tokyo').format('YYYY年MM月DD日 HH時mm分ss秒');  
        });
        ...
        console.info(`閲覧されました: user: ${req.user}, ` + `trackingId: ${trackingId}, ` + `remoteAddress: ${req.connection.remoteAddress}, ` + `user-agent: ${req.headers['user-agent']} ` + `oneTimeToken: ${oneTimeToken}`);
      });
      break;
    case 'POST':
      // TODO POST の処理
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        if (!matchResult) {
          ...
          const requestedOneTimeToken = matchResult[2];
          if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
            console.info(`削除したidは ${decoded} です`);
